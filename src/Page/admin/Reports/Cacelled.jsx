...
const HavePenaltys = ({ setTotalReserve, DashInfo }) => {
  ...
  const getFilteredItems = () => {
    if (!DashInfo.cancelledDetails) return [];
    
    const today = new Date();
    today.setHours(0, 0, 0, 0);

    return DashInfo.cancelledDetails.filter(item => {
  ...
  const generatePDF = () => {
    ...
    const approvedItems = filteredItems.filter(item => item.status === "APPROVED BY ADMIN");
    const waitingForRefundItems = filteredItems.filter(item => item.status === "WAITING FOR REFUND");
    // Summary section
    yPosition += 15;
    doc.setFontSize(16);
    doc.setFont(undefined, 'bold');
        ...
        <div className="flex items-center justify-between p-6 border-b border-gray-700">
          <div>
            ...
            <p className="text-sm text-gray-400 mt-1">
              {filteredItems.length || 0} {activeFilter === 'today' ? "today's cancellations" : 'historical cancellations'}
            </p>
          </div>
            ...
            <div className="text-center py-12">
              <p className="text-gray-400">
                {activeFilter === 'today' 
                  ...
                  : "No historical cancellations found"}
              </p>
            </div>
          )}
        </div>
