...
const Appointment = () =>{
   ...
   useEffect(()=>{
    ...
    async function appointment(){
        ...
        try{
            const response =  await fetch(`https://backend-production-024f.up.railway.app/appoinmentID/${ID.id}`,{
                method:'get',
                headers:{
   ...
   useEffect(()=>{
     ...
     const socket = io.connect('https://backend-production-024f.up.railway.app'); // Use your server's URL

    async function ALLappointment(){

        try{
            const response =  await fetch(`https://backend-production-024f.up.railway.app/allAppointments`,{
                method:'get',
                headers:{
                    'Content-Type':'application/json'
                }
