 ...
 const EditSize = () =>{
    ...
    const [measurements2, setMeasurements2] = useState({
      ...
      const handleEdit = async(e) => {
        ...
        try{
            const response = await fetch(`https://backend-production-024f.up.railway.app/editsize/${ID2.id}`,{
                method:'PUT',
      ...
      useEffect(() =>{
        ...
        const valueDefault = async() =>{
            ...
            try{
                const respionse = await fetch(`https://backend-production-024f.up.railway.app/getSize/${ID2.id}`,{
                   method:"GET",
                   headers:{
                       'Content-Type':'application/json'
                   }
