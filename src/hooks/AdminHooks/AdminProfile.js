    ...
    setUserPro(pros => ({
        ...
        [type]: value
          ...
          useEffect(()=>{
            ...
            async function YourProfile() {
                  try{
                      const response = await fetch(`https://backend-production-d6a2.up.railway.app/AdminProfile/${ADID.id}`,{
                        method:"GET",
                        headers:{
       ...
       //!Tnanggal ko d2 yung AIID.id
        ...
        const Edits = async (e) => {
          ...
          try {
              const response = await fetch(`https://backend-production-d6a2.up.railway.app/user_can_Edit/${ADID.id}`, {
                  method: "PUT",
