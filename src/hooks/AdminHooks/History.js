...
const HistoryOfUser = (ID) =>{
    ...
    useEffect(() => {
      ...
      const fetchData = async () => {
        try {
          const response = await fetch(`https://backend-production-024f.up.railway.app/ApprovedItems/${ID}`);
    ...
    const deletedItems = async(id) =>{
       try{
       const response = await fetch(`https://backend-production-024f.up.railway.app/removeGrace/${id}`,{
        method:"DELETE",
        headers:{
