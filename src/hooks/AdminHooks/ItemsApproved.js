  ...
  const PushToApprove = async(e,pro) =>{
    e.preventDefault();
      ...
      try{
          ...
          const response = await fetch(`https://backend-production-62ff.up.railway.app/ItemsApproved`,{
               method:"PUT",
...
const DELETE = async(e,pro) =>{
  e.preventDefault();
    ...
    try{
        const response = await fetch(`https://backend-production-62ff.up.railway.app/removeIncheck/${pro.id}`,{
             method:"delete",
             headers:{
                'Content-Type':'application/json'
             }
