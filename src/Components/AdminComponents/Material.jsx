...
const Material = () => {
    ...
    const { material, setMaterial } = updateCategory();
    ...
    const handleUpdate = async(e,id) => {
      ...
      e.preventDefault();
      try{
        const response = await fetch(`https://backend-production-024f.up.railway.app/MaterialEdit/${id}`,{
          method:"put",
          headers:{
              'Content-Type':'application/json'
          },
          body:JSON.stringify({material:editCategory})
        });
        if(!response.ok) return console.log("NO DELETE");
          //!PAG ARALAN MO TO
      setMaterial((prevCategories) =>
      ...
      const handleDelete = async(e,id) => {
        ...
        try{
        const response = await fetch(`https://backend-production-024f.up.railway.app/MaterialDelete/${id}`,{
          method:"delete",
          headers:{
              'Content-Type':'application/json'
          }
        ...
        setMaterial(pro => pro.filter(element => element.id !== id));
        ...
        <tbody className="divide-y divide-gray-200 bg-white">
          {material?.map((item) => (
            <tr key={item.id} className="hover:bg-gray-50">
              ...
              <td className="whitespace-nowrap px-6 py-4">
                 {
                    item.id === byID ? field : (<div className="text-sm text-gray-900">{item.material}</div>)
                  }
              </td>
